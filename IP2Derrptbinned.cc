...
void IP2Derrptbinned(){
  ...
  for(int ii=0; ii<5; ii++) {
    ...
    double seltrk_ip2d, seltrk_pvdz;
    double pt, eta, phi;
    double seltrk_ip2dsig;

    t2->SetBranchAddress("seltrk_ip2d", &seltrk_ip2d);
    ...
    TH1F* ip2derr_incl = new TH1F("ip2derr_incl", "ip2derr", 400, -2, 2);
    TH1F* ip2derr_pt3 = new TH1F("ip2derr_pt3", "ip2derr", 400, -2, 2);
    TH1F* ip2derr_pt14 = new TH1F("ip2derr_pt14", "ip2derr", 400, -2, 2);

    TH1F* ip2derr_etaH_pt3 = new TH1F("ip2derr_etaH_pt3", "ip2derr", 400, -2, 2);
    TH1F* ip2derr_etaL_pt3 = new TH1F("ip2derr_etaL_pt3", "ip2derr", 400, -2, 2);

    TH1F* ip2derr_phiU_pt3 = new TH1F("ip2derr_phiU_pt3", "ip2derr", 400, -2, 2);
    TH1F* ip2derr_phiD_pt3 = new TH1F("ip2derr_phiD_pt3", "ip2derr", 400, -2, 2);

    for(int i=0; i<t2->GetEntries(); i++){
      if(i%10000000==0) std::cout << (float) i*100./(float) t2->GetEntries() << "%% processed" << std::endl;
      t2->GetEntry(i);
      double seltrk_ip2derr = seltrk_ip2dsig==0? 0 : seltrk_ip2d/seltrk_ip2dsig;
      ip2derr_incl->Fill(seltrk_ip2derr);
      if(pt > 3. && pt < 4.) { 
        ip2derr_pt3->Fill(seltrk_ip2derr);
        if( abs(eta)>1.5 ) ip2derr_etaH_pt3->Fill(seltrk_ip2derr);
        if( abs(eta)<1.5 ) ip2derr_etaL_pt3->Fill(seltrk_ip2derr);
        if( phi>0. ) ip2derr_phiU_pt3->Fill(seltrk_ip2derr);
        if( phi<0. ) ip2derr_phiD_pt3->Fill(seltrk_ip2derr);
      }
      if(pt > 14. && pt < 15.) {
        ip2derr_pt14->Fill(seltrk_ip2derr);
      }
    }
    ...
    ip2derr_pt3->Sumw2();
    ip2derr_pt14->Sumw2();
    ip2derr_etaH_pt3->Sumw2();
    ip2derr_etaL_pt3->Sumw2();
    ip2derr_phiU_pt3->Sumw2();
    ip2derr_phiD_pt3->Sumw2();
    ip2derr_incl->Scale(1./ip2derr_incl->Integral());
    ip2derr_pt3->Scale(1./ip2derr_pt3->Integral());
    ip2derr_pt14->Scale(1./ip2derr_pt14->Integral());
    ip2derr_etaH_pt3->Scale(1./ip2derr_etaH_pt3->Integral());
    ip2derr_etaL_pt3->Scale(1./ip2derr_etaL_pt3->Integral());
    ip2derr_phiU_pt3->Scale(1./ip2derr_phiU_pt3->Integral());
    ip2derr_phiD_pt3->Scale(1./ip2derr_phiD_pt3->Integral());

    TCanvas *c1 = MakeCanvas(ip2derr_incl, ip2derr_pt3, ip2derr_pt14, "IP2D Err", "1", inputnames[ii]);
    c1->SaveAs(("IP2DErrptbinned_"+inputnames[ii]+"_seltrk_pt.pdf").c_str());
    TCanvas *c2 = MakeCanvas(ip2derr_pt3, ip2derr_etaH_pt3, ip2derr_etaL_pt3, "IP2D Err", "2", inputnames[ii]);
    c2->SaveAs(("IP2DErrptbinned_"+inputnames[ii]+"_seltrk_eta.pdf").c_str());
    TCanvas *c3 = MakeCanvas(ip2derr_pt3, ip2derr_phiU_pt3, ip2derr_phiD_pt3, "IP2D Err", "3", inputnames[ii]);
    c3->SaveAs(("IP2DErrptbinned_"+inputnames[ii]+"_seltrk_phi.pdf").c_str());
  }
