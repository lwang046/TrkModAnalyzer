...
TCanvas * MakeCanvas(TH1F *h1, TH1F *h2, TH1F *h4=NULL, bool dofit=false, std::string xtitle="xaxis title", std::string canv=""){
   ...
   if(h4!=NULL) {
     ...
     legend->AddEntry(h1,"MC mult. jet l","pl");
     legend->AddEntry(h2,"MC mult. jet b","pl");
   }
   legend->Draw();
...
void trackpar(){
  ...
  for(int i=0; i<t2->GetEntries(); i++){
    if(i%1000000==0) std::cout << (float) i*100./(float) t2->GetEntries() << "%% processed" << std::endl;
    t2->GetEntry(i);
    if(btag_2<0.1){
  ...
  for(int i=0; i<t3->GetEntries(); i++){
    if(i%1000000==0) std::cout << (float) i*100./(float) t3->GetEntries() << "%% processed" << std::endl;
    t3->GetEntry(i);
    if(btag_3<0.1){
  ...
  c1->SaveAs("ljet_ip2d.pdf");
  c2->SaveAs("bjet_ip2d.pdf");
  c3->SaveAs("ljet_pvdz.pdf");
  c4->SaveAs("bjet_pvdz.pdf");
  c5->SaveAs("lvbip2d.pdf");
  c6->SaveAs("lvbpvdz.pdf");
